...
useEffect(() => {
    ...
    .then((res) => {
        ...
        setHighlight(first.highlights["0"]);
...
//    const htmlWithoutIframes = html.replace(/<iframe[\s\S]*?<\/iframe>/gi, "");
  ...
  {Object.values(recent).map((article: Article, index: number) => (
    <Pressable key={article.ID ?? index} style={{flexDirection:"row",justifyContent:"center",backgroundColor:"#FFF",paddingTop:20,paddingRight:10,paddingBottom:10,marginRight:5,marginBottom:5,marginLeft:5,borderBottomWidth:0.2,borderColor:"#444",shadowOpacity:0.1,elevation:2}}>
